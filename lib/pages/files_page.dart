...
class _FilesPageState extends State<FilesPage> {
  ...
  Widget _buildFileList(BuildContext context) {
    ...
    if (_currentNodes.isEmpty && _currentPath.isEmpty) {
      // 首次加载，加载项目根
      WidgetsBinding.instance.addPostFrameCallback((_) {
        _loadDirectory(context.read<ProjectProvider>().rootPath);
      });
      return const Center(child: CircularProgressIndicator());
    }
  ...
  Future<void> _openFileEditor(String path) async {
    try {
      ...
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => _FileEditorPage(path: path, content: content),
        ),
      );
    } catch (e) {
      if (!mounted) return;
...
class _FileEditorPageState extends State<_FileEditorPage> {
  ...
  Future<void> _save() async {
    try {
      final project = context.read<ProjectProvider>();
      await project.fileService.writeFile(widget.path, _controller.text);
